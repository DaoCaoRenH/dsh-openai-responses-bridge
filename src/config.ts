...
export function assertServiceable(config: BridgeConfig): void {
  for (const [route, source] of Object.entries(config.providers ?? {})) {
    ...
    if (hosted?.imageGeneration?.enabled === true) {
      ...
      throw new Error(`llm-openai-responses-bridge: provider "${route}" imageGeneration requires a verified DSH assistant-image output backend; none is available in rc.7`)
    }
