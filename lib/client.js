window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function bindSnapshotSelector(source) {
			...
			return function useSelector(selector, equality) {
				const selected = selector((0, react.useSyncExternalStore)(subscribe, getSnapshot, getSnapshot));
				const cached = (0, react.useRef)(void 0);
				const equal = equality ?? Object.is;
				if (cached.current === void 0 || !equal(cached.current.value, selected)) cached.current = { value: selected };
		...
		function jsonValueOf(value) {
			...
			if (Array.isArray(value)) return value.map((item) => item === void 0 ? null : jsonValueOf(item));
			if (typeof value === "object") {
				const result = {};
				for (const [key, item] of Object.entries(value)) if (item !== void 0) result[key] = jsonValueOf(item);
				return result;
			}
		...
		*/
		function providerEditOps(route, profile, draft) {
			...
			const ops = [
				...
				},
				{
					op: "set",
					path: [...base, "models"],
					value: jsonValueOf(draft.models.map(modelProfileFromDraft))
				}
			];
		...
		var BridgeSettingsStore = class {
			...
			async load() {
				...
				try {
					const [settingsResponse, providersResponse] = await Promise.all([this.api.settings.describe(), this.api.llm.listProviders()]);
					...
					const routes = routeEntries(namespace, providersResponse.ok ? providersResponse.value : []);
		...
		const css$1 = ".iGzXhq_section{max-width:760px;color:var(--dsw-alias-label-primary);flex-direction:column;gap:16px;padding:24px 28px 40px;display:flex}.iGzXhq_sectionHeader,.iGzXhq_cardHeader,.iGzXhq_cardFooter,.iGzXhq_providerIdentity,.iGzXhq_actions,.iGzXhq_switchRow,.iGzXhq_switchControl{align-items:center;display:flex}.iGzXhq_sectionHeader,.iGzXhq_cardHeader,.iGzXhq_cardFooter{justify-content:space-between;gap:16px}.iGzXhq_sectionTitle,.iGzXhq_cardTitle{color:var(--dsw-alias-label-primary);margin:0;font-weight:600}.iGzXhq_sectionTitle{font-size:20px;line-height:28px}.iGzXhq_cardTitle{font-size:15px;line-height:22px}.iGzXhq_intro,.iGzXhq_empty,.iGzXhq_loading,.iGzXhq_notice,.iGzXhq_cardSubtitle,.iGzXhq_field small,.iGzXhq_switchRow small{color:var(--dsw-alias-label-tertiary);margin:0;font-size:13px;line-height:20px}.iGzXhq_providerList{flex-direction:column;gap:12px;display:flex}.iGzXhq_providerCard,.iGzXhq_addCard{background:var(--dsw-alias-bg-module-platform);box-shadow:0 1px 0 var(--dsw-alias-border-l1);border-radius:14px;flex-direction:column;gap:16px;padding:16px;display:flex}.iGzXhq_addCard{border:1px solid var(--dsw-alias-border-l2)}.iGzXhq_providerIdentity{align-items:flex-start;gap:10px;min-width:0}.iGzXhq_statusDot{border-radius:50%;flex:none;width:8px;height:8px;margin-top:7px}.iGzXhq_statusDotActive{background:var(--dsw-alias-state-success-primary)}.iGzXhq_statusDotDormant{background:var(--dsw-alias-label-tertiary)}.iGzXhq_badge{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:2px 8px;font-size:12px;line-height:18px}.iGzXhq_cardSubtitle code,.iGzXhq_summaryGrid dd{font-family:var(--ds-font-family-code)}.iGzXhq_summaryGrid{border-top:1px solid var(--dsw-alias-border-l1);border-bottom:1px solid var(--dsw-alias-border-l1);grid-template-columns:repeat(2,minmax(0,1fr));gap:10px 16px;margin:0;padding:12px 0;display:grid}.iGzXhq_summaryGrid div{min-width:0}.iGzXhq_summaryGrid dt{color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:16px}.iGzXhq_summaryGrid dd{color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;margin:2px 0 0;font-size:12px;line-height:18px;overflow:hidden}.iGzXhq_stateText{min-width:0;color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:12px;line-height:18px;overflow:hidden}.iGzXhq_switchRow,.iGzXhq_switchControl{gap:9px}.iGzXhq_switchRow{background:var(--dsw-alias-bg-layer-1);border-radius:10px;align-items:flex-start;padding:10px 12px}.iGzXhq_switchRow span{flex-direction:column;gap:2px;display:flex}.iGzXhq_switchControl{color:var(--dsw-alias-label-secondary);cursor:pointer;flex:none;font-size:12px;line-height:18px}.iGzXhq_formGrid{grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;display:grid}.iGzXhq_field{min-width:0;color:var(--dsw-alias-label-secondary);flex-direction:column;gap:5px;font-size:12px;line-height:18px;display:flex}.iGzXhq_fieldWide{grid-column:1/-1}.iGzXhq_input{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);width:100%;min-height:34px;color:var(--dsw-alias-label-primary);font:inherit;border-radius:8px;outline:none;padding:6px 10px}.iGzXhq_input:focus{border-color:var(--dsw-alias-brand-primary)}.iGzXhq_input:disabled,.iGzXhq_switchControl input:disabled,.iGzXhq_switchRow input:disabled{opacity:.55;cursor:default}.iGzXhq_fieldset{border:1px solid var(--dsw-alias-border-l1);border-radius:10px;margin:0;padding:14px}.iGzXhq_fieldset legend{color:var(--dsw-alias-label-secondary);padding:0 5px;font-size:12px}.iGzXhq_actions{justify-content:flex-end;gap:8px}.iGzXhq_primaryButton,.iGzXhq_secondaryButton,.iGzXhq_addButton{min-height:36px;font:inherit;cursor:pointer;border-radius:18px;padding:0 14px;font-size:13px;line-height:20px}.iGzXhq_primaryButton{background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground);border:0}.iGzXhq_secondaryButton{border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-primary);background:0 0}.iGzXhq_dangerButton{border:1px solid var(--dsw-alias-state-error-primary);min-height:36px;color:var(--dsw-alias-state-error-primary);font:inherit;cursor:pointer;background:0 0;border-radius:18px;padding:0 14px;font-size:13px;line-height:20px}.iGzXhq_addButton{border:1px dashed var(--dsw-alias-border-l3);width:100%;color:var(--dsw-alias-label-secondary);background:0 0}.iGzXhq_primaryButton:hover:not(:disabled),.iGzXhq_secondaryButton:hover:not(:disabled),.iGzXhq_dangerButton:hover:not(:disabled),.iGzXhq_addButton:hover:not(:disabled){filter:brightness(.98)}.iGzXhq_dangerButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover-danger)}.iGzXhq_primaryButton:focus-visible,.iGzXhq_secondaryButton:focus-visible,.iGzXhq_dangerButton:focus-visible,.iGzXhq_addButton:focus-visible,.iGzXhq_input:focus-visible,.iGzXhq_switchControl input:focus-visible,.iGzXhq_switchRow input:focus-visible{outline:2px solid var(--dsw-alias-border-l3);outline-offset:2px}.iGzXhq_primaryButton:disabled,.iGzXhq_secondaryButton:disabled,.iGzXhq_dangerButton:disabled,.iGzXhq_addButton:disabled{opacity:.5;cursor:default}.iGzXhq_error{color:var(--dsw-alias-state-error-primary);margin:0;font-size:12px;line-height:18px}.iGzXhq_deleteDialog{width:min(480px,100%)}.iGzXhq_deleteConfirm:not(:disabled){border-color:var(--dsw-alias-state-error-primary);color:var(--dsw-alias-state-error-primary)}.iGzXhq_deleteConfirm:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover-danger)}.iGzXhq_editor{flex-direction:column;gap:14px;display:flex}.iGzXhq_editorHeader{align-items:baseline;gap:8px;display:flex}.iGzXhq_editorTitle{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:500;line-height:22px}.iGzXhq_editorRoute{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px}.iGzXhq_fieldLabel{color:var(--dsw-alias-label-secondary);align-items:center;gap:10px;font-size:12px;font-weight:500;line-height:18px;display:inline-flex}.iGzXhq_staticInput{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);min-height:34px;color:var(--dsw-alias-label-secondary);font:inherit;border-radius:8px;align-items:center;padding:6px 10px;display:flex}.iGzXhq_advancedHint{color:var(--dsw-alias-label-tertiary);margin:0;font-size:12px;line-height:18px}.iGzXhq_editorActions{justify-content:flex-end;gap:8px;display:flex}.iGzXhq_modelCatalog{border-top:1px solid var(--dsw-alias-border-l2);flex-direction:column;gap:10px;padding-top:12px;display:flex}.iGzXhq_modelListHead{justify-content:space-between;align-items:flex-start;gap:12px;display:flex}.iGzXhq_modelCatalogHeading{flex-direction:column;gap:2px;display:flex}.iGzXhq_modelCatalogTitle{color:var(--dsw-alias-label-secondary);font-size:12px;font-weight:500;line-height:18px}.iGzXhq_modelCatalogMeta,.iGzXhq_modelEmpty{color:var(--dsw-alias-label-tertiary);margin:0;font-size:12px;line-height:18px}.iGzXhq_modelList{flex-direction:column;gap:8px;display:flex}.iGzXhq_modelEntry{border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:6px}.iGzXhq_modelRow{grid-template-columns:minmax(0,1.4fr) minmax(0,1fr) auto auto;align-items:center;gap:6px;display:grid}.iGzXhq_iconButton{box-sizing:border-box;width:28px;height:28px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:6px;justify-content:center;align-items:center;padding:0;display:inline-flex}.iGzXhq_iconButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.iGzXhq_iconButtonDanger:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover-danger);color:var(--dsw-alias-state-error-primary)}.iGzXhq_iconButton:disabled{cursor:default;opacity:.4}.iGzXhq_modelAdvanced{grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;padding:10px 2px 2px;display:grid}.iGzXhq_modelField{flex-direction:column;gap:5px;min-width:0;display:flex}.iGzXhq_modelFieldLabel{color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:16px}.iGzXhq_linkButton,.iGzXhq_addModelButton{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);min-height:28px;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;background:0 0;border-radius:14px;justify-content:center;align-items:center;gap:5px;padding:0 10px;font-size:12px;line-height:18px;display:inline-flex}.iGzXhq_linkButton:hover:not(:disabled),.iGzXhq_addModelButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.iGzXhq_addModelButton{border-style:dashed;border-radius:8px;width:100%;min-height:36px}.iGzXhq_linkButton:disabled,.iGzXhq_addModelButton:disabled{cursor:default;opacity:.4}.iGzXhq_candidateList{flex-direction:column;gap:8px;max-height:320px;margin:0;padding:0;list-style:none;display:flex;overflow:auto}.iGzXhq_candidate{border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:8px 10px}.iGzXhq_candidateLabel{color:var(--dsw-alias-label-primary);cursor:pointer;align-items:center;gap:8px;display:flex}.iGzXhq_candidateId{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.iGzXhq_input:focus-visible,.iGzXhq_staticInput:focus-visible,.iGzXhq_linkButton:focus-visible,.iGzXhq_addModelButton:focus-visible,.iGzXhq_iconButton:focus-visible{outline:2px solid var(--dsw-alias-border-l3);outline-offset:2px}@media (width<=640px){.iGzXhq_section{padding:18px 16px 32px}.iGzXhq_formGrid,.iGzXhq_summaryGrid{grid-template-columns:1fr}.iGzXhq_fieldWide{grid-column:auto}.iGzXhq_modelRow{grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto auto}.iGzXhq_modelAdvanced{grid-template-columns:repeat(2,minmax(0,1fr))}.iGzXhq_cardFooter{flex-direction:column;align-items:flex-start}}";
		...
		var BridgeSection_module_css_default = {
			"actions": "iGzXhq_actions",
			"addButton": "iGzXhq_addButton",
			"addCard": "iGzXhq_addCard",
			"addModelButton": "iGzXhq_addModelButton",
			"advancedHint": "iGzXhq_advancedHint",
			"badge": "iGzXhq_badge",
			"candidate": "iGzXhq_candidate",
			"candidateId": "iGzXhq_candidateId",
			"candidateLabel": "iGzXhq_candidateLabel",
			"candidateList": "iGzXhq_candidateList",
			"cardFooter": "iGzXhq_cardFooter",
			"cardHeader": "iGzXhq_cardHeader",
			"cardSubtitle": "iGzXhq_cardSubtitle",
			"cardTitle": "iGzXhq_cardTitle",
			"dangerButton": "iGzXhq_dangerButton",
			"deleteConfirm": "iGzXhq_deleteConfirm",
			"deleteDialog": "iGzXhq_deleteDialog",
			"editor": "iGzXhq_editor",
			"editorActions": "iGzXhq_editorActions",
			"editorHeader": "iGzXhq_editorHeader",
			"editorRoute": "iGzXhq_editorRoute",
			"editorTitle": "iGzXhq_editorTitle",
			"empty": "iGzXhq_empty",
			"error": "iGzXhq_error",
			"field": "iGzXhq_field",
			"fieldLabel": "iGzXhq_fieldLabel",
			"fieldset": "iGzXhq_fieldset",
			"fieldWide": "iGzXhq_fieldWide",
			"formGrid": "iGzXhq_formGrid",
			"iconButton": "iGzXhq_iconButton",
			"iconButtonDanger": "iGzXhq_iconButtonDanger",
			"input": "iGzXhq_input",
			"intro": "iGzXhq_intro",
			"linkButton": "iGzXhq_linkButton",
			"loading": "iGzXhq_loading",
			"modelAdvanced": "iGzXhq_modelAdvanced",
			"modelCatalog": "iGzXhq_modelCatalog",
			"modelCatalogHeading": "iGzXhq_modelCatalogHeading",
			"modelCatalogMeta": "iGzXhq_modelCatalogMeta",
			"modelCatalogTitle": "iGzXhq_modelCatalogTitle",
			"modelEmpty": "iGzXhq_modelEmpty",
			"modelEntry": "iGzXhq_modelEntry",
			"modelField": "iGzXhq_modelField",
			"modelFieldLabel": "iGzXhq_modelFieldLabel",
			"modelList": "iGzXhq_modelList",
			"modelListHead": "iGzXhq_modelListHead",
			"modelRow": "iGzXhq_modelRow",
			"notice": "iGzXhq_notice",
			"primaryButton": "iGzXhq_primaryButton",
			"providerCard": "iGzXhq_providerCard",
			"providerIdentity": "iGzXhq_providerIdentity",
			"providerList": "iGzXhq_providerList",
			"secondaryButton": "iGzXhq_secondaryButton",
			"section": "iGzXhq_section",
			"sectionHeader": "iGzXhq_sectionHeader",
			"sectionTitle": "iGzXhq_sectionTitle",
			"stateText": "iGzXhq_stateText",
			"staticInput": "iGzXhq_staticInput",
			"statusDot": "iGzXhq_statusDot",
			"statusDotActive": "iGzXhq_statusDotActive",
			"statusDotDormant": "iGzXhq_statusDotDormant",
			"summaryGrid": "iGzXhq_summaryGrid",
			"switchControl": "iGzXhq_switchControl",
			"switchRow": "iGzXhq_switchRow"
		};
		//#endregion
		...
		*/
		...
		function AddCustomProviderCard({ namespace, existingRoutes, writable, api, t, mode = "create", route, profile: initialProfile, credentialConfigured = false, onCancel, onSaved }) {
			...
			const save = async () => {
				...
				try {
					...
					if (!committed) {
						const response = await api.settings.mutate(namespace.ns, editing ? providerEditOps(draft.route.trim(), existingProfile, draft) : [{
							op: "set",
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(BridgeModelListEditor, {
							...
							probe: {
								...
								...editing && route !== void 0 ? { provider: route } : {},
		...
		function ProviderSummaryCard({ route, profile, credentialRef, credential, active, namespace, writable, api, t, onChanged }) {
			...
			const toggle = async (enabled) => {
				...
				setFailure(void 0);
				try {
					const response = await api.settings.mutate(namespace.ns, webSearchOps(route, profile, enabled), namespace.revision);
					if (!response.ok) {
						setFailure(response.error.code === "settings-conflict" ? t("conflict") : response.error.message);
						return;
					}
			...
			const confirmDelete = async () => {
				...
				try {
					...
					const response = await api.settings.mutate(namespace.ns, providerDeleteOps(route), namespace.revision);
					if (!response.ok) {
		...
		const css = ".YfeUtq_root{width:100%;color:var(--dsw-text-primary)}.YfeUtq_row{min-height:24px}.YfeUtq_leading{color:var(--dsw-text-secondary)}.YfeUtq_title{font-weight:500}.YfeUtq_chevron{color:var(--dsw-text-tertiary)}.YfeUtq_separator{background:var(--dsw-text-tertiary);opacity:.65;border-radius:50%;width:3px;height:3px;margin:0 7px}.YfeUtq_summary,.YfeUtq_status{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.YfeUtq_summary{max-width:360px;color:var(--dsw-text-secondary)}.YfeUtq_status{color:var(--dsw-text-tertiary)}.YfeUtq_body{padding:4px 0 8px 28px}.YfeUtq_meta{color:var(--dsw-text-tertiary);margin-bottom:6px;font-size:12px}.YfeUtq_error{color:var(--dsw-color-error,#c43d3d);white-space:pre-wrap;margin:8px 0 0}";
		...
		var HostedWebSearchCard_module_css_default = {
			"body": "YfeUtq_body",
			"chevron": "YfeUtq_chevron",
			"error": "YfeUtq_error",
			"leading": "YfeUtq_leading",
			"meta": "YfeUtq_meta",
			"root": "YfeUtq_root",
			"row": "YfeUtq_row",
			"separator": "YfeUtq_separator",
			"status": "YfeUtq_status",
			"summary": "YfeUtq_summary",
			"title": "YfeUtq_title"
		};
		//#endregion
		...
		function HostedWebSearchCard({ data, labels }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.DisclosureRow, {
					...
					collapsedContent: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						query !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
		...
		function HostedWebSearchNodeView({ node, t }) {
			const labels = {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HostedWebSearchCard, {
				data: node.data,
				labels
			});
		}
		...
		function registerHostedWebSearchConversationNode(ctx) {
			ctx.get("uiConversation").events.register(hostedWebSearchDefinition);
