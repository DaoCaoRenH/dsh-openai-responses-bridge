...
function assertServiceable(config) {
	for (const [route, source] of Object.entries(config.providers ?? {})) {
		...
		if (hosted?.imageGeneration?.enabled === true) throw new Error(`llm-openai-responses-bridge: provider "${route}" imageGeneration requires a verified DSH assistant-image output backend; none is available in DSH 0.1.2-alpha.2`);
...
*/
async function discoverModels(request, storedApiKey, signal) {
...
* facts as safe to preserve. The Bridge Host must be loaded before persisted
...
function sourceValues(...values) {
	...
	const add = (value) => {
		...
		if (source === void 0 || result.length >= MAX_SOURCES$1) return;
...
function citationOf(value) {
	const annotation = recordOf$1(value);
...
function annotationsOf(value) {
	const item = recordOf$1(value);
	if (item === void 0) return [];
	const content = Array.isArray(item.content) ? item.content : [];
	const result = [];
	for (const contentItem of content) {
		const contentRecord = recordOf$1(contentItem);
...
var HostedWebSearchObserver = class {
	...
	states() {
		return [...this.entries.values()].map((entry) => ({
			...
			...entry.state.error === void 0 ? {} : { error: { ...entry.state.error } }
		}));
	}
	observeUnsafe(raw) {
		const event = recordOf$1(raw);
		if (event === void 0) return;
		const type = typeof event.type === "string" ? event.type : "";
		this.sequence += 1;
		if (type === "response.created") {
			const response = recordOf$1(event.response);
			this.responseId = idOf(response?.id, event.response_id) ?? this.responseId;
			this.updateResponseId();
			return;
		}
		if (type === "response.output_item.added") {
			const item = recordOf$1(event.item);
		...
		if (type.startsWith("response.web_search_call.")) {
			...
			this.applyItem(entry, recordOf$1(event.item) ?? event, event);
			return;
		}
		if (type === "response.completed" || type === "response.incomplete") {
			...
			for (const itemValue of output) {
				const item = recordOf$1(itemValue);
				if (this.isSearchItem(item)) {
		...
		if (type === "response.failed" || type === "error") {
			const error = this.errorOf(event, recordOf$1(event.response)?.error);
			this.responseTerminal = true;
	...
	entryFor(event, item) {
		const action = recordOf$1(item?.action) ?? recordOf$1(event.action);
		...
		const rawId = itemId ?? (outputIndex === void 0 ? void 0 : `bridge-search-${this.responseId ?? "response"}-${outputIndex}`) ?? `bridge-search-${this.responseId ?? "response"}-${this.sequence}`;
		const id = this.options.idPrefix === void 0 ? rawId : `${this.options.idPrefix}:${rawId}`;
	...
	errorOf(event, responseError) {
		const error = recordOf$1(event.error) ?? recordOf$1(responseError);
...
//#endregion
//#region src/hosted-web-search/transport.ts
...
function createResponsesClient(model, options) {
...
async function* observedStream(source, observer, signal) {
	try {
		...
		observer.finish(signal?.aborted === true ? "aborted" : "failed", {
			message: signal?.aborted === true ? "Hosted web search request aborted" : errorMessage$1(error),
	...
	(async () => {
		...
		try {
			const client = createResponsesClient(model, {
				...
				...options?.sessionId === void 0 ? {} : { session: options.sessionId }
			});
			...
			observer.finish(options?.signal?.aborted === true ? "aborted" : "failed", {
				message: options?.signal?.aborted === true ? "Request was aborted" : errorMessage$1(error),
			...
			output.errorMessage = errorMessage$1(error);
			stream.push({
				type: "error",
				reason: output.stopReason,
				error: output
...
//#endregion
//#region src/hosted-web-search/executor.ts
const MAX_QUERIES = 4;
const MAX_SOURCES = 8;
const SEARCH_TOOL_TYPES = /* @__PURE__ */ new Set(["web_search", "web_search_preview"]);
...
function hostedSearchDefinitions(hosted) {
	return (hosted.definitions ?? [{ type: "web_search" }]).filter((definition) => SEARCH_TOOL_TYPES.has(definition.type)).map((definition) => ({ ...definition }));
}
function uniqueStrings(values) {
	return [...new Set(values)];
}
function parseQueries(args) {
	const raw = recordOf(args)?.queries;
	if (!Array.isArray(raw) || raw.length === 0) throw new Error("web_search requires a non-empty queries array");
	const queries = [];
	for (const item of raw) {
		if (typeof item !== "string" || item.trim().length === 0) throw new Error("web_search queries must contain only non-empty strings");
...
function textFromResponse(value) {
	...
	return chunks.length === 0 ? void 0 : chunks.join("");
}
...
function sourceForCanonical(source) {
	return {
		url: source.url,
...
function sourceStates(states) {
	const result = [];
	const seen = /* @__PURE__ */ new Set();
	for (const state of states) for (const source of state.sources) {
...
function mergeQueryResults(results) {
	...
	const contentParts = results.filter((result) => result.content !== void 0 && result.content.length > 0).map((result) => results.length === 1 ? result.content : `### ${result.query}\n\n${result.content}`);
	return {
...
async function executeOne(query, index, options, signal) {
	const location = options.session === void 0 ? {
		turn: 0,
		step: 0
	} : activeTurnStep(options.session);
	...
	const observer = new HostedWebSearchObserver({
		...
		...location,
		queries: [query],
		...options.searchIdPrefix === void 0 ? {} : { idPrefix: `${options.searchIdPrefix}:${index}` },
		onCheckpoint: (kind, state) => {
			checkpointQueue = checkpointQueue.then(() => {
				appendHostedWebSearchCheckpoint(options.session, `bridge/hosted-web-search/${kind}`, state);
			});
		}
	});
	...
	const include = uniqueStrings([...options.hostedTools.include ?? [], "web_search_call.action.sources"]);
	const params = {
		model: options.model.id,
		input: [{
			role: "user",
	...
	try {
		...
		const { data } = await createResponsesClient(options.model, { apiKey: options.apiKey }).responses.create(params, {
			signal,
			maxRetries: 0
		}).withResponse();
	...
	return {
		query,
		...deltaText.length > 0 ? { content: deltaText } : fallbackText === void 0 ? {} : { content: fallbackText },
...
async function executeHostedWebSearch(args, options) {
	const queries = parseQueries(args);
	if (queries.length === 1) return mergeQueryResults([await executeOne(queries[0], 0, options, options.signal)]);
...
function errorInfo(error) {
	...
	const code = typeof record?.code === "string" && record.code.length > 0 ? record.code : void 0;
	if (code === void 0) return void 0;
	return {
		name: error instanceof Error && error.name.length > 0 ? error.name : "BridgeHostedWebSearchError",
		code
	};
}
...
function modelFor(profile, modelId) {
	return profile.piProvider.getModels().find((model) => model.id === modelId);
}
function isNestedRunCodeSearch(exec, activeRunCodes) {
	return exec.name === "web_search" && exec.parent !== void 0 && activeRunCodes.has(exec.parent);
}
/** Install the PTC nested-dispatch bridge without registering a second tool. */
function installPtcHostedWebSearch(ctx, options) {
	const activeRunCodes = /* @__PURE__ */ new Set();
	ctx.on("tools/execute", async (exec, next) => {
		...
		if (target === void 0) return failureResult(/* @__PURE__ */ new Error("Bridge could not determine the active provider and model for PTC web search"));
		const source = options.current().providers?.[target.provider];
		const api = source?.api ?? "openai-responses";
		if (source === void 0 || api !== "openai-responses" || !hostedWebSearchEnabled(source.hostedTools)) return next();
		const profile = options.profiles().get(target.provider);
		if (profile === void 0) return failureResult(/* @__PURE__ */ new Error(`Bridge provider route "${target.provider}" is not available`));
		const model = modelFor(profile, target.model);
		if (model === void 0) return failureResult(/* @__PURE__ */ new Error(`Bridge provider route "${target.provider}" has no model "${target.model}"`));
		try {
			const apiKey = await options.resolveApiKey(target.provider, profile);
			return {
				isError: false,
				value: await executeHostedWebSearch(exec.arguments, {
					model,
					apiKey,
					hostedTools: source.hostedTools,
					...exec.agent?.session === void 0 ? {} : { session: exec.agent.session },
...
function apply(ctx, config) {
	...
	ctx.llm.registerModelDiscovery(NS, (request, signal) => discoverModels(request, request.provider === void 0 ? void 0 : async () => {
